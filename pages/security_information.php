...
<section class="w-full pt-8 pb-6 mb-8 border-b">
...
<div class="flex border-b">
    <button data-tab="lost_and_found" class="tab-button w-1/3 py-3 font-semibold <?= $active_tab==='lost_and_found'?'active':'' ?>">
        <i class="fa-solid fa-box-open mr-2"></i> Barang
    </button>
    <button data-tab="alerts" class="tab-button w-1/3 py-3 font-semibold <?= $active_tab==='alerts'?'active':'' ?>">
        <i class="fa-solid fa-bell mr-2"></i> Peringatan
    </button>
    <button data-tab="map" class="tab-button w-1/3 py-3 font-semibold <?= $active_tab==='map'?'active':'' ?>">
...
<!-- BARANG -->
<div id="content-lost_and_found" class="tab-content" style="<?= $active_tab==='lost_and_found'?'':'display:none' ?>">
...
<div class="p-4 rounded-xl border-l-4 shadow bg-white <?= $item['status']==='hilang'?'border-red-600':'border-green-600' ?>">
    <div class="font-bold text-lg"><?= htmlspecialchars($item['item_name']) ?></div>
    <p class="text-sm">Lokasi: <?= htmlspecialchars($item['location']) ?></p>
    <p class="text-xs text-gray-500">Pelapor: <?= htmlspecialchars($item['reporter_email']) ?></p>
    <p class="text-sm mt-2"><?= htmlspecialchars($item['description']) ?></p>
</div>
<?php endforeach; ?>
</div>
<?php endif; ?>
...
<!-- ALERT -->
<div id="content-alerts" class="tab-content" style="<?= $active_tab==='alerts'?'':'display:none' ?>">
...
<!-- MAP -->
<div id="content-map" class="tab-content" style="<?= $active_tab==='map'?'':'display:none' ?>">
<h3 class="text-2xl font-bold text-blue-900 mb-4">Peta Keamanan Kampus</h3>
<div id="mapContainer" class="rounded-xl overflow-hidden border shadow h-[450px] bg-gray-100 flex items-center justify-center">
